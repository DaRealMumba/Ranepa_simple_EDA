...
""")
  ...
  ('mos_houses.csv', 'cars.csv', 'SaintP_houses.csv'))
  if optionReg == 'SaintP_houses.csv':
    input_Reg = pd.read_csv(optionReg, parse_dates=['first_day_exposition'])  #, sep=','
  else:
...
if st.checkbox('Показать Датасет'):
  number = st.number_input('Сколько колонок показать', min_value=1, max_value=my_data.shape[1])
  st.dataframe(my_data.head(number))

if st.checkbox('Наименование колонок'):
  st.dataframe(my_data.columns)
...
if st.checkbox('Уникальные значения целевой переменной'):
  ...
  st.dataframe(my_data.iloc[:, -1:].value_counts())
...
if st.checkbox('Типы данных'):
  ...
  ''')
  st.dataframe(my_data.dtypes.astype('str'))
